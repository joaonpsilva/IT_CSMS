...
class OCPP_Server:
    ...
    async def start_server(self, port, security_profile):
        ...
        if security_profile >= 2:
            ...
            ssl_cert = ChargePoint.server_variables["ssl_cert"]
            ssl_key = ChargePoint.server_variables["ssl_key"]
            ssl_context.load_cert_chain(ssl_cert, keyfile=ssl_key)
        
        if security_profile == 3:
            ssl_context.verify_mode = ssl.CERT_REQUIRED
            ssl_context.load_verify_locations(ChargePoint.server_variables["root_cert"])
